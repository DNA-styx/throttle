...
class Stats
{
    ...
    {
        $data = (int) $app['db']->executeQuery(
            'SELECT COUNT(*) AS count FROM crash WHERE timestamp > DATE_SUB(NOW(), INTERVAL 24 HOUR)'
        )->fetchColumn(0);
    ...
    {
        $data = (int) $app['db']->executeQuery('SELECT COUNT(*) AS count FROM crash')->fetchColumn(0);
    ...
    public function unique(Application $app)
    {
        $query = $app['db']->executeQuery(
            'SELECT COUNT(*) AS count FROM (SELECT DISTINCT COALESCE(CAST(server_id AS CHAR), CONCAT(\'owner:\', owner_id), CONCAT(\'anon:\', ip)) AS server_key FROM crash WHERE timestamp > DATE_SUB(NOW(), INTERVAL 24 HOUR)) AS _'
        );
        $data = (int) $query->fetchColumn(0);
    ...
    public function latest(Application $app, $module = null, $function = null)
    {
        ...
        if ($function !== null) {
            ...
            if (preg_match('/^%?(?:[0-9a-f]{8})+%?$/', $module)) {
                $query = $app['db']->executeQuery('SELECT crash, rendered, cmdline, NULL AS avatar FROM frame JOIN crash ON id = crash AND crash.thread = frame.thread AND crash.stackhash LIKE ? WHERE frame = 0 ORDER BY timestamp DESC LIMIT ' . $limit, array($module));
            } else {
                $query = $app['db']->executeQuery('SELECT crash, rendered, cmdline, NULL AS avatar FROM frame JOIN crash ON id = crash AND crash.thread = frame.thread WHERE frame = 0 AND module LIKE ? ORDER BY timestamp DESC LIMIT ' . $limit, array($module));
            }
        } else {
            $query = $app['db']->executeQuery('SELECT crash, rendered, cmdline, NULL AS avatar FROM crash JOIN frame ON crash = id AND frame.thread = crash.thread AND frame = 0 ORDER BY timestamp DESC LIMIT ' . $limit);
        }
