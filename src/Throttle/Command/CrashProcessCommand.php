...
*/
                    ...
                    foreach (new \LinesOfALargeExecFuture($future) as $line) {
                        ...
                        if ($data[1] == '0' && ($crashThread == $data[0] || ($crashThread == -1 && $data[0] == '0'))) {
                            $app['db']->executeUpdate(
                                'UPDATE crash SET crashmodule = ?, crashfunction = ? WHERE id = ?',
                                array(
                                    self::truncateSummaryField($data[2], 255),
                                    self::truncateSummaryField(strlen($data[3]) ? $data[3] : $data[6], 255),
                                    $id,
                                )
                            );
                        }
    ...
    private static function truncateSummaryField(?string $value, int $limit): ?string
    {
        if ($value === null) {
            return null;
        }
