...
class CrashCleanCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $groups = $app['db']->executeQuery('SELECT owner_id, ip AS display_ip FROM crash WHERE lastview IS NULL OR lastview < DATE_SUB(NOW(), INTERVAL 90 DAY) GROUP BY owner_id, ip HAVING (owner_id IS NULL AND COUNT(*) > 50) OR (owner_id IS NOT NULL AND COUNT(*) > 100)');

        while ($group = $groups->fetch()) {
            if ($group['owner_id'] !== null) {
                $query = $app['db']->executeQuery('SELECT id FROM crash WHERE owner_id = ? AND ip = ? AND (lastview IS NULL OR lastview < DATE_SUB(NOW(), INTERVAL 90 DAY)) ORDER BY timestamp DESC LIMIT 100 OFFSET 100', array($group['owner_id'], $group['display_ip']));
            } else {
                $query = $app['db']->executeQuery('SELECT id FROM crash WHERE owner_id IS NULL AND ip = ? AND (lastview IS NULL OR lastview < DATE_SUB(NOW(), INTERVAL 90 DAY)) ORDER BY timestamp DESC LIMIT 100 OFFSET 50', array($group['display_ip']));
            }
            ...
            $output->writeln('Deleted ' . $count . ' crash dumps for server: ' . ($group['owner_id'] ?: 'NULL') . ', ' . $group['display_ip']);
            $total_count += $count;
        }
